...
import { apiFetch, getSession, downloadRecu, downloadFactureLigne } from "@/lib/client/session";
...
export default function LocataireHomePage() {
  ...
  // Rent: the line carries the flat annual amount. After each payment the
  // tenant is owed three figures — paid this month, paid this year, remaining.
  const loyers = lignes.filter((l) => isLoyer(l.facture.type));
          ...
          <Card key={l.id} className="mb-6 p-4">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              ...
              <button
                ...
                onClick={() => downloadFactureLigne(l.id, `loyer-${annee}`)}
              >
                {t("facturePdf")}
              </button>
            </div>
                ...
                <tr key={l.id} className="border-b border-slate-100 last:border-0">
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      onClick={() =>
                        downloadFactureLigne(l.id, `${l.facture.type}-${l.facture.mois}`)
                      }
                    >
                      {t("facturePdf")}
                    </button>
